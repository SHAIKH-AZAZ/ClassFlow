...
- `apps/worker`: Node worker that downloads Zoom recordings and uploads them to Google Drive.
...
4. Google Drive resource metadata, recording webhook, worker job pipeline.
...
3. Create a Zoom Server-to-Server OAuth app and set webhook event subscriptions for `recording.completed`.
