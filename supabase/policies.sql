...
stable
as $$
  select current_profile_role() = 'faculty'
$$;
...
stable
as $$
  select current_profile_role() = 'student'
$$;
...
create policy "faculty_profiles_self_or_admin_read" on faculty_profiles
  for select using (
    ...
    or exists (
      ...
      where l.faculty_id = faculty_profiles.id and sp.user_id = auth.uid()
    )
  );
...
create policy "student_profiles_self_or_admin_read" on student_profiles
  for select using (
    ...
    or exists (
      ...
      join lectures l on l.group_id = gs.group_id
      join faculty_profiles fp on fp.id = l.faculty_id
      where gs.student_id = student_profiles.id and fp.user_id = auth.uid()
    )
  );
...
create policy "group_students_member_read" on group_students
  for select using (
    ...
    or exists (
      ...
      where sp.id = group_students.student_id and sp.user_id = auth.uid()
    )
    or exists (
      select 1 from lectures l
      join faculty_profiles fp on fp.id = l.faculty_id
      where l.group_id = group_students.group_id and fp.user_id = auth.uid()
    )
  );
...
create policy "zoom_meetings_read" on zoom_meetings
  for select using (
    ...
    or exists (
      select 1 from lectures l
      join faculty_profiles fp on fp.id = l.faculty_id
      where l.id = zoom_meetings.lecture_id and fp.user_id = auth.uid()
    )
    or exists (
      select 1 from lectures l
      join group_students gs on gs.group_id = l.group_id
      join student_profiles sp on sp.id = gs.student_id
      where l.id = zoom_meetings.lecture_id and sp.user_id = auth.uid()
    )
  );
...
create policy "attendance_overrides_read" on attendance_overrides
  for select using (
    ...
    or exists (
      ...
      join faculty_profiles fp on fp.id = l.faculty_id
      where a.id = attendance_overrides.attendance_id and fp.user_id = auth.uid()
    )
    or exists (
      select 1 from attendance a
      join student_profiles sp on sp.id = a.student_id
      where a.id = attendance_overrides.attendance_id and sp.user_id = auth.uid()
    )
  );
...
create policy "recordings_read" on recordings
  for select using (
    ...
    or exists (
      select 1 from lectures l
      join faculty_profiles fp on fp.id = l.faculty_id
      where l.id = recordings.lecture_id and fp.user_id = auth.uid()
    )
    or exists (
      select 1 from lectures l
      join group_students gs on gs.group_id = l.group_id
      join student_profiles sp on sp.id = gs.student_id
      where l.id = recordings.lecture_id and sp.user_id = auth.uid()
    )
  );
...
-- recording_jobs (admin-only; service role bypasses)
...
create policy "resources_group_visible" on resources
  for select using (
    ...
    or exists (
      ...
      from lectures l
      join faculty_profiles fp on fp.id = l.faculty_id
      where l.group_id = resources.group_id and fp.user_id = auth.uid()
    )
  );
...
create policy "resources_admin_faculty_write" on resources
  for all using (
    is_admin()
    or owner_id = auth.uid()
    or current_profile_role() = 'faculty'
  ) with check (
    is_admin()
    or owner_id = auth.uid()
    or current_profile_role() = 'faculty'
...
create policy "chat_threads_admin_write" on chat_threads
  for update using (is_admin() or created_by = auth.uid()) with check (is_admin() or created_by = auth.uid());
...
create policy "chat_thread_members_read" on chat_thread_members
  for select using (
    ...
    or exists (
      select 1 from chat_thread_members me
...
create policy "notices_read" on notices
  for select using (
    is_admin()
    or notices.group_id is null
    or exists (
      select 1 from group_students gs
      join student_profiles sp on sp.id = gs.student_id
      where gs.group_id = notices.group_id and sp.user_id = auth.uid()
    )
    or exists (
      select 1 from lectures l
      join faculty_profiles fp on fp.id = l.faculty_id
      where l.group_id = notices.group_id and fp.user_id = auth.uid()
    )
  );
...
create policy "remarks_read" on remarks
  for select using (
    ...
    or exists (
      select 1 from student_profiles sp
      where sp.id = remarks.student_id and sp.user_id = auth.uid()
    )
    or exists (
      select 1 from faculty_profiles fp
      where fp.id = remarks.faculty_id and fp.user_id = auth.uid()
    )
    or exists (
      select 1 from faculty_profiles fp
      join lectures l on l.faculty_id = fp.id
      join group_students gs on gs.group_id = l.group_id
      where gs.student_id = remarks.student_id and fp.user_id = auth.uid()
    )
  );
